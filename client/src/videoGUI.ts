...
export default class Video {
  ...
  createMenu(): void {
    ...
    const createNewRoom = this.createMenuButton(
      "createNewRoom",
      "CREATING NEW ROOM"
    );

    createNewRoom.onPointerClickObservable.add(async () => {
      this.swapControls(false);
      await this.createGame("createNewRoom");
    });
    stackPanel.addControl(createNewRoom);
  ...
  private createMenuButton(name: string, text: string): GUI.Button {
    ...
    // };
    // const ANote0Video = BABYLON.MeshBuilder.CreatePlane("plane", planeOpts);
